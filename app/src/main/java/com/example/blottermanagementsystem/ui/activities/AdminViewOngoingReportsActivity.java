...
public class AdminViewOngoingReportsActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            setContentView(R.layout.activity_admin_view_inprogress_reports);
            ...
            android.util.Log.e("AdminViewOngoing", "Error in onCreate: " + e.getMessage(), e);
        }
    }
    ...
    private void initializeViews() {
        try {
            ...
            android.util.Log.e("AdminViewOngoing", "Error initializing views: " + e.getMessage());
        }
    }
    ...
    private void setupToolbar() {
        ...
        if (toolbar != null) {
            ...
            if (getSupportActionBar() != null) {
                ...
                getSupportActionBar().setTitle("Ongoing Reports");
            }
        ...
        try {
            ...
            if (chipAll != null) {
                chipAll.setOnCheckedChangeListener((buttonView, isChecked) -> {
                    if (isChecked) navigateToScreen(AdminViewAllReportsActivity.class);
                });
            }
            if (chipPending != null) {
                chipPending.setOnCheckedChangeListener((buttonView, isChecked) -> {
                    if (isChecked) navigateToScreen(AdminViewPendingReportsActivity.class);
                });
            }
            if (chipOngoing != null) {
            ...
            if (chipAssigned != null) {
                chipAssigned.setOnCheckedChangeListener((buttonView, isChecked) -> {
                    if (isChecked) navigateToScreen(AdminViewAssignedReportsActivity.class);
                });
            }
            if (chipResolved != null) {
                chipResolved.setOnCheckedChangeListener((buttonView, isChecked) -> {
                    if (isChecked) navigateToScreen(AdminViewResolvedReportsActivity.class);
                });
            }
            if (chipClosed != null) {
                chipClosed.setOnCheckedChangeListener((buttonView, isChecked) -> {
                    if (isChecked) navigateToScreen(AdminViewClosedReportsActivity.class);
                });
            }
            if (btnSort != null) {
            ...
            android.util.Log.e("AdminViewOngoing", "Error setting up listeners: " + e.getMessage());
        }
    }
    ...
    private void navigateToScreen(Class<?> activityClass) {
        try {
            ...
            android.util.Log.e("AdminViewOngoing", "Error navigating: " + e.getMessage());
        }
    }
    ...
    private void loadReportsFromApi() {
        ApiClient.getAllReports(new ApiClient.ApiCallback<List<BlotterReport>>() {
            ...
            public void onSuccess(List<BlotterReport> apiReports) {
                BlotterDatabase db = BlotterDatabase.getDatabase(AdminViewOngoingReportsActivity.this);
                new Thread(() -> {
                    try {
                        ...
                        android.util.Log.e("AdminViewOngoing", "Error saving API data: " + e.getMessage());
                        loadReportsFromDatabase();
                    }
                }).start();
            }
            ...
            public void onError(String errorMessage) {
                android.util.Log.w("AdminViewOngoing", "API error: " + errorMessage);
                loadReportsFromDatabase();
            }
        });
    }
    ...
    private void loadReportsFromDatabase() {
        new Thread(() -> {
            try {
                ...
                android.util.Log.e("AdminViewOngoing", "Error loading from database: " + e.getMessage());
            }
        }).start();
    }
        ...
        new Thread(() -> {
            try {
                ...
                for (BlotterReport report : allSystemReports) {
                    ...
                    else if ("ONGOING".equals(status) || "IN PROGRESS".equals(status)) ongoing++;
                ...
                android.util.Log.e("AdminViewOngoing", "Error updating statistics: " + e.getMessage());
            }
        }).start();
    }
    ...
    private void updateEmptyState() {
        if (filteredReports.isEmpty()) {
            ...
            if (emptyStateIcon != null) emptyStateIcon.setImageResource(R.drawable.ic_ongoing_modern);
            if (emptyStateTitle != null) emptyStateTitle.setText("No Ongoing Reports");
            if (emptyStateMessage != null) emptyStateMessage.setText("Cases currently being investigated\nwill appear here.");
        } else {
            if (emptyStateCard != null) emptyStateCard.setVisibility(View.VISIBLE);
    ...
    private void loadReportsQuietly() {
        new Thread(() -> {
            try {
                List<BlotterReport> reports = database.blotterReportDao().getAllReports();
                List<BlotterReport> ongoingReports = new ArrayList<>();
                for (BlotterReport report : reports) {
                    String status = report.getStatus() != null ? report.getStatus().toUpperCase() : "";
                    // Show active cases: ASSIGNED, ONGOING, IN PROGRESS (but NOT RESOLVED or CLOSED)
                    if ("ASSIGNED".equals(status) || "ONGOING".equals(status) || "IN PROGRESS".equals(status)) {
                        ongoingReports.add(report);
                    }
                }
                boolean dataChanged = hasDataChanged(ongoingReports, allReports);
                if (dataChanged) {
                    runOnUiThread(() -> {
                        allReports.clear();
                        allReports.addAll(ongoingReports);
                        filterReports();
                        updateStatistics();
                    });
                }
            } catch (Exception e) {
                android.util.Log.e("AdminViewOngoing", "Error in quiet refresh: " + e.getMessage());
            }
        }).start();
    }
