...
public class AdminViewResolvedReportsActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            setContentView(R.layout.activity_admin_view_resolved_reports);
            ...
            android.util.Log.e("AdminViewResolved", "Error in onCreate: " + e.getMessage(), e);
        }
    }
    ...
    private void initializeViews() {
        try {
            ...
            android.util.Log.e("AdminViewResolved", "Error initializing views: " + e.getMessage());
        }
    }
        ...
        try {
            ...
            if (chipOngoing != null) {
                chipOngoing.setOnCheckedChangeListener((buttonView, isChecked) -> {
                    if (isChecked) navigateToScreen(AdminViewOngoingReportsActivity.class);
                });
            }
            ...
            android.util.Log.e("AdminViewResolved", "Error setting up listeners: " + e.getMessage());
        }
    }
    ...
    private void navigateToScreen(Class<?> activityClass) {
        try {
            ...
            android.util.Log.e("AdminViewResolved", "Error navigating: " + e.getMessage());
        }
    }
    ...
    private void loadReportsFromApi() {
        ApiClient.getAllReports(new ApiClient.ApiCallback<List<BlotterReport>>() {
            ...
            public void onSuccess(List<BlotterReport> apiReports) {
                BlotterDatabase db = BlotterDatabase.getDatabase(AdminViewResolvedReportsActivity.this);
                new Thread(() -> {
                    try {
                        ...
                        android.util.Log.e("AdminViewResolved", "Error saving API data: " + e.getMessage());
                        loadReportsFromDatabase();
                    }
                }).start();
            }
            ...
            public void onError(String errorMessage) {
                android.util.Log.w("AdminViewResolved", "API error: " + errorMessage);
                loadReportsFromDatabase();
            }
        });
    }
    ...
    private void loadReportsFromDatabase() {
        new Thread(() -> {
            try {
                ...
                android.util.Log.e("AdminViewResolved", "Error loading from database: " + e.getMessage());
            }
        }).start();
    }
    ...
    private void updateStatistics() {
        new Thread(() -> {
            try {
                ...
                android.util.Log.e("AdminViewResolved", "Error updating statistics: " + e.getMessage());
            }
        }).start();
    }
    ...
    private void updateEmptyState() {
        if (filteredReports.isEmpty()) {
            ...
            if (emptyStateIcon != null) emptyStateIcon.setImageResource(R.drawable.ic_resolved_modern);
            if (emptyStateTitle != null) emptyStateTitle.setText("No Resolved Reports");
            if (emptyStateMessage != null) emptyStateMessage.setText("Cases that have been resolved\nwill appear here.");
        } else {
            if (emptyStateCard != null) emptyStateCard.setVisibility(View.VISIBLE);
    ...
    private void loadReportsQuietly() {
        new Thread(() -> {
            try {
                List<BlotterReport> reports = database.blotterReportDao().getAllReports();
                List<BlotterReport> resolvedReports = new ArrayList<>();
                ...
                boolean dataChanged = hasDataChanged(resolvedReports, allReports);
                if (dataChanged) {
                    runOnUiThread(() -> {
                        allReports.clear();
                ...
                android.util.Log.e("AdminViewResolved", "Error in quiet refresh: " + e.getMessage());
            }
        }).start();
    }
