...
public class ViewAllHearingsActivity extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            android.util.Log.e("ViewAllHearings", "Error in onCreate: " + e.getMessage(), e);
    ...
    private void setupToolbar() {
        ...
        if (toolbar != null) {
            ...
            if (getSupportActionBar() != null) {
                ...
                getSupportActionBar().setTitle("All Hearings");
            }
            toolbar.setNavigationOnClickListener(v -> {
                android.util.Log.d("ViewAllHearings", "Back button clicked");
                finish();
            });
        } else {
            android.util.Log.e("ViewAllHearings", "Toolbar not found!");
        }
    }
    
    @Override
    public boolean onSupportNavigateUp() {
        android.util.Log.d("ViewAllHearings", "onSupportNavigateUp called");
        finish();
        return true;
    }
    ...
    private void setupChips() {
        ...
        if (chipAll != null) {
            ...
            chipAll.setOnCheckedChangeListener((buttonView, isChecked) -> {
                if (isChecked) {
                    // Already on All screen, no navigation needed
                    android.util.Log.d("ViewAllHearings", "All chip selected - staying on current screen");
                }
            });
        }
    ...
    private void navigateToScreen(Class<?> activityClass) {
        android.util.Log.d("ViewAllHearings", "Navigating to: " + activityClass.getSimpleName());
    ...
    private void loadHearings() {
        android.util.Log.d("ViewAllHearings", "Loading all hearings");
        
        new Thread(() -> {
            try {
                BlotterDatabase db = BlotterDatabase.getDatabase(this);
                List<Hearing> hearings = db.hearingDao().getAllHearings();
                
                runOnUiThread(() -> {
                    allHearings.clear();
                    allHearings.addAll(hearings);
                    filterHearings("");
                    android.util.Log.d("ViewAllHearings", "Loaded " + hearings.size() + " hearings");
                });
            } catch (Exception e) {
                android.util.Log.e("ViewAllHearings", "Error loading hearings: " + e.getMessage(), e);
                runOnUiThread(() -> {
    ...
    private void updateEmptyState() {
        if (filteredHearings.isEmpty()) {
            ...
            // Update empty state content for All hearings
            if (emptyStateIcon != null) {
                emptyStateIcon.setImageResource(R.drawable.ic_calendar_filled);
            }
            if (emptyStateTitle != null) {
                emptyStateTitle.setText("No Hearings Scheduled");
            }
            if (emptyStateMessage != null) {
                emptyStateMessage.setText("Hearings will appear here\\nwhen scheduled.");
            }
        } else {
    ...
    private void loadHearingsQuietly() {
        ...
        new Thread(() -> {
            try {
                ...
                android.util.Log.e("ViewAllHearings", "Error in quiet refresh: " + e.getMessage());
            }
        }).start();
    }
}
