...
public class ViewOngoingReportsActivity extends BaseActivity {
    ...
    private void navigateToScreen(Class<?> activityClass) {
        try {
            ...
            android.util.Log.e("ViewOngoingReports", "Navigation error: " + e.getMessage(), e);
    ...
    private void filterReports() {
        ...
        // Filter for ONGOING reports - show ONLY ongoing/in progress cases
        for (BlotterReport report : allReports) {
            String status = report.getStatus() != null ? report.getStatus().toUpperCase() : "";
            // Show ONLY: ONGOING, IN PROGRESS (NOT ASSIGNED - that's in Assigned chip)
    ...
    private void updateEmptyState() {
        if (filteredReports.isEmpty()) {
            ...
            // Keep sort button visible even when empty state
            if (btnSort != null) btnSort.setVisibility(View.VISIBLE);
            // Layout already has: Cyan background, chart icon, "No Ongoing Reports" title
        } else {
            ...
            // Keep sort button visible when data exists
            if (btnSort != null) btnSort.setVisibility(View.VISIBLE);
        }
    }
